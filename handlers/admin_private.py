...
from database.orm_query import orm_check_user, orm_update_status, orm_get_users


admin_private_router = Router()
admin_private_router.message.filter(ChatTypesFilter(['private']), IsAdmin())

#TODO: Запись состояния пользователя в бд, чтобы он получал статус из БД.
...
async def get_user_list(message: Message, session: AsyncSession):
    for product in await orm_get_users(session, user_status="send_photo_state"):
